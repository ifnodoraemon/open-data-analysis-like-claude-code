...
- Treat successful `report_finalize` as a delivery boundary for the current run. After it returns `ok=true`, `is_finalized=true`, and `delivery_state=finalized`, the runtime may ask the model for one final user-facing response with tools disabled, then complete the run. Do not replace that final response with a hardcoded template.
