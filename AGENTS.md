...
- Treat successful `report_finalize` as a delivery boundary for the current run. After it returns `ok=true`, `is_finalized=true`, and `delivery_state=finalized`, the runtime should complete the run instead of asking the model for another round.
